class MiqExpression
  ...
  BASE_TABLES = CONFIG[:base_tables].freeze
  INCLUDE_TABLES = CONFIG[:include_tables].freeze
  EXCLUDE_COLUMNS = CONFIG[:exclude_columns].freeze
  EXCLUDE_EXCEPTIONS = CONFIG[:exclude_exceptions].freeze
  TAG_CLASSES = CONFIG[:tag_classes].freeze
  EXCLUDE_FROM_RELATS = CONFIG[:exclude_from_relats].freeze
  FORMAT_SUB_TYPES = CONFIG[:format_sub_types].transform_values do |sub_type|
    sub_type.transform_values do |string_or_array|
      if string_or_array.kind_of?(String)
        _(string_or_array)
      else
        string_or_array.collect { |abbr, unit| [_(abbr), unit] }
      end
    end
  end.freeze
